...
const url = "mongodb://login:password@localhost:27017/";
const app = express();
const port = 8080;
...
mongoClient.connect(function (err, client) {
    ...
    reportsCollection.find().toArray(function (err, results) {
        reports = results;
...
function addReport(report) {
    ...
    if (report.hasOwnProperty("account")
        ...
        reportsCollection.insertOne(report, function (err, result) {
            if (err) {
...
function addUser(name, email, password) {
    ...
    if (!userPresent) {
        ...
        const user = {name: name, email: email, password: password, id: userId++};
        users.push(user);
